...
class MSGraphAuth:
    ...
    def check_health(self):
        ...
        pass_token = self._load_refresh_token()
        env_token = os.environ.get("GRAPH_REFRESH_TOKEN", "").strip()
        if pass_token:
    ...
    def _write_refresh_token(self, token):
        try:
            ...
            if result.returncode != 0:
                raise RuntimeError(f"pass write failed: {result.stderr}")
        except FileNotFoundError:
            raise RuntimeError("'pass' CLI not found (install with: brew install pass)")
