...
echo ""
echo -e "${BOLD}MyOS — Uninstall${RESET}"
echo ""
...
if [ "$(uname)" = "Darwin" ]; then
  ...
  found_plists=0
  for plist in "$LAUNCH_DIR"/com.myos.*.plist \
               "$LAUNCH_DIR"/com.claudeclaw.*.plist \
               "$LAUNCH_DIR"/com.myos.plist \
               "$LAUNCH_DIR"/com.claudeclaw.plist; do
    [ -f "$plist" ] || continue
    found_plists=1
...
if [ "$(uname)" = "Linux" ] && command -v systemctl &>/dev/null; then
  ...
  found_units=0
  for unit in "$SYSTEMD_DIR"/myos*.service "$SYSTEMD_DIR"/claudeclaw*.service; do
    [ -f "$unit" ] || continue
    found_units=1
    name=$(basename "$unit")
...
# ── 3. Remove config directory (~/.myos) ──────────────────────────
CONFIG_DIR="${MYOS_CONFIG:-${CLAUDECLAW_CONFIG:-$HOME/.myos}}"
# Also check .env for a custom path
if [ -f "$PROJECT_ROOT/.env" ]; then
  env_config=$(grep '^MYOS_CONFIG=' "$PROJECT_ROOT/.env" 2>/dev/null | cut -d'=' -f2- | sed "s|^~|$HOME|")
...
# ── 5. Remove temp files ────────────────────────────────────────────────
rm -f /tmp/myos*.log /tmp/myos*.err /tmp/claudeclaw*.log /tmp/claudeclaw*.err /tmp/warroom-debug.log /tmp/warroom-agents.json
...
echo ""
echo -e "  ${GREEN}✓${RESET}  ${BOLD}MyOS uninstalled.${RESET}"
echo ""
