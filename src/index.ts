...
const LEGACY_PID_FILE = AGENT_ID === 'main' ? path.join(STORE_DIR, 'claudeclaw.pid') : null;
...
function acquireLock(): void {
  ...
  try {
    for (const pidFile of [PID_FILE, LEGACY_PID_FILE].filter(Boolean) as string[]) {
  ...
  if (LEGACY_PID_FILE) {
    try { fs.unlinkSync(LEGACY_PID_FILE); } catch { /* ignore */ }
  }
}
