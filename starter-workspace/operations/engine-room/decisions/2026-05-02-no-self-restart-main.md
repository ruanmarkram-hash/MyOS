...
`launchctl kickstart -k com.claudeclaw.main` (or any equivalent SIGTERM/SIGKILL of the main bot process) MUST NOT be invoked from inside an active Telegram conversation. Three layers enforce this:
...
Sage's pattern was: do code work, run `npm test`, commit, then `launchctl kickstart -k com.claudeclaw.main` to "verify the new code is live". The kickstart SIGTERMed the very process streaming Sage's final reply to Telegram. Result: ~5 minutes of progress events visible to [YOUR NAME], then nothing — the assistant turn never committed to `conversation_log`, the Telegram send was cut mid-flush.
