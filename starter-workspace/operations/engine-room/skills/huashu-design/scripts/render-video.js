...
(async () => {
  ...
  if (hasReady) {
    // Second safeguard: explicitly seek animation time to zero — handles cases where
    // HTML does not strictly follow the starter tick template (e.g. lastTick uses
    // performance.now() causing font-load time to bleed into the first frame's dt).
