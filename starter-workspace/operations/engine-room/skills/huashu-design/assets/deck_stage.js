...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Shadow DOM renders first (independent of child nodes, not affected by parser timing)
      this._render();
      ...
      // so querySelectorAll would return empty.
      // Defer to next event loop tick to ensure all children have been parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document still parsing — wait for DOMContentLoaded to catch all sections at once
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document already parsed (script at bottom of body or deferred) — collect on next frame
