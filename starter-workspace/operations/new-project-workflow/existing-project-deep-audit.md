...
**Purpose:** Define what a real existing-project audit looks like when the new-project-workflow runs in existing-project mode. The shallow 20-line audit that was produced for Example Doc Generator is NOT the target — that project had no code yet. This guide is for projects that have substantial code already written and need to be mapped against "production ready."

**Context:** Written 2026-04-11 after running a real deep audit on Example App (56 views, 88 Supabase tables, 256+ db.ts functions). The Example App audit surfaced ~19 BLOCKS_PROD gaps across 6 domains in a single parallel pass, replacing a week of manual click-through testing. Reference output at `projects/example-app/audits/YYYY-MM-DD/`.
...
When Example App was audited, five structural patterns recurred across domains. Every deep audit should start by hunting for these specific patterns. They're the classes of failure that manual testing surfaces one at a time and that audits can catch all at once.
...
- `// TODO`, `// not yet`, `// Mason will` near wizard submit handlers
...
A library file (`esig.ts`, `email.ts`, `sa-generator.ts`) is fully implemented. A table exists in the schema. But no view imports the library or calls the function. Comments like `// Mason will build the UI` are a tell.
...
One agent covering the whole codebase is too slow and too shallow. The audit must be parallel across domains. For Example App, 6 domains worked well:
...
4. **Operational core** — the primary day-to-day workflow (scheduling, notes, payments for Example App)
...
### "Readme-driven audit"
Reading the brief/roadmap and assuming claims are true. Always re-verify against the code. The Example App sprint log claimed "GAP-01 through GAP-09 resolved" — re-verification found 3 of those were half-resolved at best.
...
### "Exit-code audit"
Running tests or `tsc --noEmit` and declaring victory if they pass. Type safety + tests passing does NOT mean features work. Example App builds in 774ms with zero errors, and the audit found 19 BLOCKS_PROD issues. The build tells you the code COMPILES. The audit tells you if it DOES what it's supposed to.

### "Claim-centric audit"
Auditing only what someone claims is broken. Example App's Cuba Service Agreement incident was one feature out of dozens that were broken — the user found one, the audit found the other 18. Always audit the whole domain, not just the reported symptom.

---

## Example App audit as reference

The 2026-04-11 Example App audit is the reference implementation of this guide. Review it before running your first deep audit:

- Input files the agents read: `projects/sonke-hub/brief.md`, `roadmap.md`, `context.md`, `sprint-log.md`, `qa-punch-list.md`, plus 6 domain-specific file lists
...
- Output files: `projects/example-app/audits/YYYY-MM-DD/{A,B,C,D,E,F}-*.md`
...
Compare this to the Example Doc Generator audit at `audit-report-ndis-doc-generator.md` — 20 lines, high-level, pre-kickoff. Both are valid outputs for their contexts; they serve different purposes.
