...
async function getEmbedding(text: string): Promise<number[]> {
  const r = await fetch(
    `${GEMINI_BASE}/models/${EMBED_MODEL}:embedContent?key=${GOOGLE_API_KEY}`,
    {
      method: "POST",
  ...
  if (!r.ok) {
    ...
    throw new Error(`Gemini embedContent failed: ${r.status} ${msg}`);
  }
  const d = await r.json();
  const values = d?.embedding?.values;
  if (!Array.isArray(values) || values.length !== EMBED_DIM) {
    throw new Error(`Gemini embedContent returned unexpected shape (len=${values?.length})`);
  }
...
Only extract what's explicitly there.`;
  ...
  const r = await fetch(
    `${GEMINI_BASE}/models/${EXTRACT_MODEL}:generateContent?key=${GOOGLE_API_KEY}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  try {
    return raw ? JSON.parse(raw) : { topics: ["uncategorized"], type: "observation" };
  } catch {
    return { topics: ["uncategorized"], type: "observation" };
  }
}
