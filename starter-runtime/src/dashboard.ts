...
// Category A — anything that needs user's hands or judgement.
...
const HUMAN_ACTION_PATTERN = new RegExp([
  ...
  // Ready-for-user-to-send patterns
...
// completion that produced a deliverable user needs to review still counts.
...
//   - mission-cli invocation by Sage (created_by='main') — user asked Sage on Telegram
...
// than an action prompt. Distinct from Category A — these don't need user
...
function defaultReviewStatusForTask(task: MissionTask, missions: MissionTask[]): MissionReviewStatus | null {
  ...
  if (task.status === 'failed' || task.status === 'partial') {
    ...
    // breakages user needs surfaced so he can retry, redirect, or archive.
  ...
  if (task.status === 'completed') {
    ...
    // Category A: anything that needs user's hands or a deliverable he asked for.
...
function buildReviewFollowupPrompt(task: MissionTask, instructions: string): string {
  return [
    ...
    '',
    `user's instructions for this pass:`,
...
function normalizeStructuredAction(input: any): StructuredBriefAction | null {
  ...
  const requiresUser = typeof input.requires_ruan === 'boolean' ? input.requires_ruan
    : typeof input.requiresUser === 'boolean' ? input.requiresUser
      : undefined;
  return {
...
function displayDetailForStructuredAction(action: StructuredBriefAction): string {
  ...
  if (typeof action.requiresUser === 'boolean') meta.push(`Requires user: ${action.requiresUser ? 'yes' : 'no'}`);
...
// so user can review the result. (2026-05-06: this used to suppress
// regardless of follow-up status, which hid completions user asked for.)
  ...
  app.post('/api/home/attention/assign', async (c) => {
    ...
    if (attentionMatch) {
      ...
      createMissionTask(id, missionTitle, [
        ...
        instruction ? `Additional instructions from user:\n${instruction}` : '',
