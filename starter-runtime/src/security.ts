    ...
    if (os.platform() === 'darwin') {
      ...
      try {
        ...
        for (const line of output.split('\n')) {
          ...
          if (label && label.startsWith('com.myos.')) {
      ...
      try {
        execSync('systemctl --user stop "com.myos.*" 2>/dev/null', { stdio: 'ignore', timeout: 3000 });
      } catch { /* ok */ }
    }
...
//     subprocess exits 1).
//
// Ported from upstream (earlyaidopters/myos-os) src/security.ts.
...
const SDK_KEEP_VARS = ['MYOS_AGENT_ID'] as const;
