...
async function runAgentTurn(args: RunAgentTurnArgs): Promise<string> {
  ...
  const sessionId = getSession(sessionChatId, agentId, provider.name) ?? undefined;
...
- ALWAYS FINALIZE WITH TEXT: Your last action of the turn must be a plain-text reply, not another tool call. Even if the work isn't fully done, end with one short paragraph saying what landed, what didn't, and what's still needed. Empty bubbles with only a tool strip are a failure case — the user can see the tools but doesn't know if you finished. Budget your tool calls so you have room to summarize before the turn closes.`;
  ...
  try {
    ...
    if (provider.name !== 'claude') {
      const systemPrompt = loadWarRoomAgentPrompt(agentId, agentDir);
      const providerMessage = buildAgentRuntimePrompt(framedText, systemPrompt, {
      ...
      const result = await provider.runAgent({
        ...
        onStreamText: (accumulatedText) => {
          const delta = accumulatedText.startsWith(lastStreamed)
            ? accumulatedText.slice(lastStreamed.length)
            : accumulatedText;
          lastStreamed = accumulatedText;
          if (delta) {
            channel.emit({ type: 'agent_chunk', turnId, agentId, role, delta });
          }
        },
        mcpAllowlist: toolPolicy.allowedMcpServers,
        cwdOverride: agentDir,
        maxTurns: agentId === 'main' ? 10 : 8,
        sandboxMode: 'read-only',
        bypassApprovalsAndSandbox: false,
      });
      newSessionId = result.newSessionId;
      fullText = result.text ?? lastStreamed;
      gotResult = !result.aborted;
      incomplete = !!result.aborted;
      saveWarRoomUsage(sessionChatId, agentId, result.usage);
    } else {
      for await (const ev of query({
  ...
  if (sessionSaveAllowed) {
    setSession(sessionChatId, newSessionId!, agentId, provider.name);
...
function loadWarRoomAgentPrompt(agentId: string, agentDir: string): string | undefined {
  const promptPath = agentId === 'main'
    ? resolveMainClaudeMdPath()
    : resolveAgentClaudeMd(agentId) ?? path.join(agentDir, 'CLAUDE.md');
  try {
    if (fs.existsSync(promptPath)) return fs.readFileSync(promptPath, 'utf-8');
  } catch (err) {
    logger.warn({ agentId, err: err instanceof Error ? err.message : err }, 'failed to read war-room agent prompt');
  }
  return undefined;
}

function saveWarRoomUsage(sessionChatId: string, agentId: string, usage: UsageInfo | null): void {
  if (!usage) return;
  try {
    saveTokenUsage(
      sessionChatId,
    ...
    logger.warn(
      { err: err instanceof Error ? err.message : err, agentId },
      'failed to persist warroom token usage (non-fatal)',
    );
  }
}
