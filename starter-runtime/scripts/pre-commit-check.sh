...
echo "Running MyOS pre-commit safety checks..."
echo ""
...
#    planning artifacts. They are already gitignored, but a forced add
#    would still slip them past ignore rules without this second check.
...
if [ -n "$WORKING_DOCS" ]; then
  ...
  echo "  These look like personal scratch work. Move them outside the repo"
  echo "  or rename intentionally if they are user-facing docs."
  FAILED=1
fi
...
if git diff --cached --name-only | grep -q "^CLAUDE.md$"; then
  if ! git show :CLAUDE.md | grep -q "\[YOUR NAME\]"; then
    echo -e "${RED}BLOCKED: CLAUDE.md does not contain user placeholders.${NC}"
