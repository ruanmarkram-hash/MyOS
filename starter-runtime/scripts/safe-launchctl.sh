...
# user sees ~5 minutes of activity then nothing. Per CLAUDE.md the rule
# is "tell user to send /restart in Telegram" — this wrapper enforces it.
#
...
for arg in "$@"; do
  ...
  if $saw_kickstart && [[ "$arg" == *"com.myos.main"* ]]; then
...
if $is_main_kickstart && [[ "${FORCE:-}" != "1" ]]; then
  cat >&2 <<'EOF'
safe-launchctl: REFUSED — cannot kickstart com.myos.main from an
...
If you want the new code live, ask user to send `/restart` in Telegram
