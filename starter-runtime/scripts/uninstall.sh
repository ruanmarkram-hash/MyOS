...
echo ""
echo -e "${BOLD}MyOS OS — Uninstall${RESET}"
echo ""
...
if [ "$(uname)" = "Darwin" ]; then
  ...
  found_plists=0
  for plist in "$LAUNCH_DIR"/com.myos.*.plist; do
    [ -f "$plist" ] || continue
    found_plists=1
...
if [ "$(uname)" = "Linux" ] && command -v systemctl &>/dev/null; then
  ...
  found_units=0
  for unit in "$SYSTEMD_DIR"/myos*.service; do
    [ -f "$unit" ] || continue
    found_units=1
    name=$(basename "$unit")
...
CONFIG_DIR="${MYOS_CONFIG:-$HOME/.myos}"
...
# ── 5. Remove temp files ────────────────────────────────────────────────
rm -f /tmp/myos*.log /tmp/myos*.err /tmp/warroom-debug.log /tmp/warroom-agents.json
...
echo ""
echo -e "  ${GREEN}✓${RESET}  ${BOLD}MyOS OS uninstalled.${RESET}"
echo ""
